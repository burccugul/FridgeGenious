...
class GeminiRecipeService {
  ...
  Future<List<String>> getIngredientsFromDatabase() async {
    ...
    return inventory.map<String>((item) => item['food_name'] as String).toList();
  }
  ...
  Future<void> saveRecipeToSupabase(Map<String, dynamic> recipeJson) async {
    try {
      ...
      if (ingredients.isEmpty || steps.isEmpty) {
        _logger.warning("Eksik veri: ingredients veya steps boş.");
        return;
      }

      await _supabaseHelper.client.from('recipes').insert({
      ...
      _logger.info("Tarif başarıyla Supabase'e eklendi.");
    } catch (e) {
      _logger.severe("Tarif eklenirken hata oluştu: $e");
